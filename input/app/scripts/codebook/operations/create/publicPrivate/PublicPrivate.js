...
class PublicPrivate {
  ...
  constructor ({
    ...
    this.userDefinedHighlighterDefinition = path
  }

  createDefaultAnnotations (callback) {
    Codebook.setAnnotationServer(null, (annotationServer) => {
      // Create annotation guide from user defined highlighter definition
      const annotationGuide = Codebook.fromObjects(this.userDefinedHighlighterDefinition)
