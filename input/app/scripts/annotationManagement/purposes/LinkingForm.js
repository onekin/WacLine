...
class LinkingForm {
    ...
    static showLinkingForm (annotation, callback){
        ...
        const html = CommentingForm.generateLinkFormHTML({ annotation })
        const swalCallback = CommentingForm.generateLinkFormCallback({ annotation, preConfirmData, sidebarStatus, callback })
        ...
        return (err, result)=> {
            if(!_.isUndefined(preConfirmData.linkAnnotation)){
                if(err){
                    ...
                    annotation.links.push(preConfirmData.linkAnnotation)
                    let bAnnotation = window.abwa.annotationManagement.annotationReader.allAnnotations.find(a => a.id === linkAnnotation)
